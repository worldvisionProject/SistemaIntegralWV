...
namespace WordVision.ec.Web.Areas.Encuesta.Controllers
{
    [Area("Encuesta")]
    //[Authorize]
    public class EMetaController : BaseController<EMetaController>
    {
        private readonly CommonMethods _commonMethods;

        public EMetaController()
        {
        ...
        {
            List<EMetaViewModel> viewModels = new List<EMetaViewModel>();
            ...
            var response = await _mediator.Send(new GetAllEMetasQuery { Include = true });

            //Mapeamos la estructura de la base a la estructura deseada tipo GetAllEEvaluacionesResponse
            if (response.Succeeded) viewModels = _mapper.Map<List<EMetaViewModel>>(response.Data);
        ...
        public async Task<JsonResult> OnGetCreateOrEdit(int id = 0)
        {
            ...
            try
            {
                ...
                var entidadViewModel = new EMetaViewModel();
                if (id == 0)
                {
                    //Llenamos los atributos select con datos traidos de la base
                    //Ponemos en el SelectList para el combo
                    var respuestaEEvaluaciones = await _mediator.Send(new GetAllEEvaluacionesQuery { Include = false });
                    if (respuestaEEvaluaciones.Succeeded && respuestaEEvaluaciones.Data != null)
                    {
                        var cmbEvaluaciones = new SelectList(respuestaEEvaluaciones.Data.ToList(), "Id", "NombreCompleto");
                        entidadViewModel.EEvaluacionList = cmbEvaluaciones;
                    }
                ...
                else
                {
                    ...
                    var response = await _mediator.Send(new GetEMetasByIdQuery() { Id = id });
                    if (response.Succeeded)
                    {
                        entidadViewModel = _mapper.Map<EMetaViewModel>(response.Data);
            ...
            {
                return _commonMethods.SaveError($"OnGetCreateOrEdit Error al consultar EMeta.", ex.Message);
            }
        }

        [HttpPost]
        public async Task<JsonResult> OnPostCreateOrEdit(EMetaViewModel EMetaViewModel)
        {
            ...
            {
                if (EMetaViewModel.OperacionEdicion == "I")
                {
                    //Modo Insercion

                    var createEntidadCommand = _mapper.Map<CreateEMetaCommand>(EMetaViewModel);
                ...
                else
                {
                    ...
                    var updateEntidadCommand = _mapper.Map<UpdateEMetaCommand>(EMetaViewModel);
                ...
                if (response.Succeeded)
                {
                    var viewModel = _mapper.Map<List<EMetaViewModel>>(response.Data);
            ...
            else
            {
                ...
                return _commonMethods.SaveError($"Error al insertar EMeta", result);
            }
        }
        ...
        public async Task<JsonResult> OnPostDelete(int id = 0)
        {
            try
            {
                var deleteCommand = await _mediator.Send(new DeleteEMetaCommand { Id = id });
            ...
            {
                _logger.LogError(ex, "OnPostDelete EMeta");
