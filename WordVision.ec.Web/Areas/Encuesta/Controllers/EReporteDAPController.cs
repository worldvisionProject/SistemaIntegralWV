...
namespace WordVision.ec.Web.Areas.Encuesta.Controllers
{
    ...
    public class EReporteDAPController : BaseController<EReporteDAPController>
    {
        public async Task<IActionResult> Index(int evaluacionID, int regionID, string provinciaID, string cantonID, string programaID, string indicadorID)
        {
            ...
            if (respuestaEEvaluaciones.Succeeded && respuestaEEvaluaciones.Data != null)
            {
                ...
                var response = await _mediator.Send(new GetAllEReporteDAPsQuery() { EvaluacionId = evaluacionID, RegionId = regionID, ProvinciaId = provinciaID, CantonId = cantonID, ProgramaId = programaID, IndicadorId = indicadorID });
                if (response.Succeeded && response.Data != null)
                {
                    //Ponemos los valores del listado que se trajo en el formato declarado en el ViewModel
                    var viewModel = _mapper.Map<List<EReporteDAPViewModel>>(response.Data);
            ...
            return View(new EReporteDAPViewModel());
        }
